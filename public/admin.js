...
const mycomponenteweb = (() => {
  const api = {
    ...
    async get(ruta) {
      const r = await fetch(`/api/get?ruta=${encodeURIComponent(ruta)}`);
      if (!r.ok) throw new Error('get_fail');
      return r.json();
    },
    async create({ ruta, texto }) {
      ...
      if (!r.ok) {
        ...
        throw new Error(e.error || 'create_fail');
      }
      return r.json();
    },
    ...
    async remove({ ruta }) {
      ...
      if (!r.ok) {
        const e = await r.json().catch(() => ({}));
        throw new Error(e.error || 'delete_fail');
      }
      return r.json();
    }
  };
  ...
  async function renderList() {
    ...
    try {
      ...
      if (!data.webs || !data.webs.length) { root.textContent = 'Sin páginas'; return; }
      ...
      root.innerHTML = `
        <table>
          <thead><tr><th>Ruta</th><th>Archivo</th><th>Acciones</th></tr></thead>
          <tbody>${rows}</tbody>
        </table>`;

      // Bind acciones
      root.querySelectorAll('[data-edit]').forEach(btn => btn.addEventListener('click', async (e) => {
        const ruta = e.currentTarget.getAttribute('data-edit');
        await openEditor(ruta);
      }));
      root.querySelectorAll('[data-del]').forEach(btn => btn.addEventListener('click', async (e) => {
        const ruta = e.currentTarget.getAttribute('data-del');
        if (!confirm(`¿Borrar /${ruta}? Esta acción no se puede deshacer.`)) return;
        try { await api.remove({ ruta }); await renderList(); notify('Borrado.'); }
        catch (err) { alert('Error al borrar: ' + err.message); }
      }));
    } catch (err) {
      root.textContent = 'Error al cargar el listado.';
      console.error(err);
    }
  }
  ...
  async function openEditor(ruta) {
    ...
    editor.innerHTML = 'Cargando contenido...';
    try {
      ...
      editor.innerHTML = `
        <div style="display:grid; gap:8px;">
          ...
          <label>HTML:
            <textarea id="editTexto">${escapeHtml(texto)}</textarea>
          </label>
          <div>
      ...
      $('#btnSave').addEventListener('click', async () => {
        const newHtml = $('#editTexto').value;
        try { await api.edit({ ruta, texto: newHtml }); notify('Guardado.'); }
        catch (err) { alert('Error al guardar: ' + err.message); }
      });
    } catch (err) {
      editor.textContent = 'No se pudo cargar el contenido.';
      console.error(err);
    }
  }
  ...
  function notify(msg) {
    ...
    n.style.bottom = '16px'; n.style.right = '16px';
    n.style.background = '#16a34a'; n.style.color = '#031321';
  ...
  window.addEventListener('DOMContentLoaded', () => {
    ...
    btn?.addEventListener('click', async () => {
      const ruta = document.getElementById('newRuta').value.trim();
      const texto = document.getElementById('newTexto').value;
      if (!ruta) return alert('Indica una ruta');
      try { await api.create({ ruta, texto }); notify('Creada.'); document.getElementById('newRuta').value=''; document.getElementById('newTexto').value=''; await renderList(); }
      catch (err) {
...
// Exponer en window para el ejemplo del enunciado
